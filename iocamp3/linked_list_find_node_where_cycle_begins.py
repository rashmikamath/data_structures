def find_node_cycle_start(l):
	...
	front= l.head
	back = l.head
	#increment one pointer to length of cycle
	for i in range(nodepass):
		front = front.next
	#Increment both pinter until it reaches same
	while front != back:
		front = front.next
		back = back.next
	return front.data
